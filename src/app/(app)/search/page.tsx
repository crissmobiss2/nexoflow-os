...
import { Search, Loader2, ArrowRight, Users, FolderKanban, BookOpen, Hash, X, Command, FileText, Building2 } from "lucide-react";
...
type ResultType = "client" | "project" | "snippet";
...
export default function SearchPage() {
  ...
  const allResults = [
    ...(grouped?.clients ?? []).map((r) => ({ ...r, type: "client" as ResultType })),
    ...(grouped?.projects ?? []).map((r) => ({ ...r, type: "project" as ResultType })),
    ...(grouped?.snippets ?? []).map((r) => ({ ...r, type: "snippet" as ResultType })),
  ];
    ...
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
          Unified search across clients, projects, and knowledge
        </p>
      </div>
      ...
      >
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search clients, projects, knowledge…"
      ...
      {!debouncedQuery ? (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-sm" style={{ color: "var(--text-muted)" }}>
            Search across clients, projects, and knowledge snippets
          </p>
        ...
        <div className="space-y-6">
          {/* Clients */}
          {(!activeType || activeType === "all" || activeType === "client") && grouped?.clients && grouped.clients.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs" style={{ color: "var(--text-muted)" }}>({grouped.clients.length})</span>
              </div>
          ...
          {/* Projects */}
          {(!activeType || activeType === "all" || activeType === "project") && grouped?.projects && grouped.projects.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs" style={{ color: "var(--text-muted)" }}>({grouped.projects.length})</span>
              </div>
              <div className="space-y-1.5">
          ...
          {/* Knowledge Snippets */}
          {(!activeType || activeType === "all" || activeType === "snippet") && grouped?.snippets && grouped.snippets.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs" style={{ color: "var(--text-muted)" }}>({grouped.snippets.length})</span>
              </div>
              <div className="space-y-1.5">
                {grouped.snippets.map((item) => (
                  <Link
                    key={`snippet-${item.id}`}
                    href={item.href}
                    className="flex items-center gap-3 px-4 py-3 rounded-xl transition-colors hover:bg-white/[0.02] group"
